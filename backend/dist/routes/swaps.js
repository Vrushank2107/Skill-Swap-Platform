...
router.post('/', [
    (0, express_validator_1.body)('responderId').notEmpty().withMessage('Responder ID is required'),
    (0, express_validator_1.body)('offeredSkillId').notEmpty().withMessage('Offered skill ID is required'),
    (0, express_validator_1.body)('wantedSkillId').notEmpty().withMessage('Wanted skill ID is required')
], async (req, res) => {
    try {
        ...
        const offeredSkill = await db.get('SELECT id, user_id, skill_name FROM skills WHERE id = ? AND user_id = ? AND approved = 1', [offeredSkillId, requesterId]);
        const wantedSkill = await db.get('SELECT id, user_id, skill_name FROM skills WHERE id = ? AND user_id = ? AND approved = 1', [wantedSkillId, responderId]);
        if (!offeredSkill) {
    ...
    `, [requesterId, responderId, responderId, requesterId, offeredSkillId, wantedSkillId]);
        ...
        const swapId = (0, uuid_1.v4)();
        await db.run('INSERT INTO swaps (id, requester_id, responder_id, offered_skill_id, wanted_skill_id) VALUES (?, ?, ?, ?, ?)', [swapId, requesterId, responderId, offeredSkillId, wantedSkillId]);
...
router.put('/:swapId/accept', async (req, res) => {
    try {
    ...
    `, [swapId, userId]);
        ...
        await db.run('UPDATE swaps SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', ['accepted', swapId]);
...
router.put('/:swapId/reject', async (req, res) => {
    try {
        const { swapId } = req.params;
        const userId = req.user?.id;
        const db = (0, init_1.getDb)();
    ...
    `, [swapId, userId]);
        ...
        await db.run('UPDATE swaps SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', ['rejected', swapId]);
        // Send notification to requester
        const io = req.app.get('io');
...
router.put('/:swapId/cancel', async (req, res) => {
    try {
        const { swapId } = req.params;
        const userId = req.user?.id;
        const db = (0, init_1.getDb)();
    ...
    `, [swapId, userId]);
        ...
        await db.run('UPDATE swaps SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', ['cancelled', swapId]);
